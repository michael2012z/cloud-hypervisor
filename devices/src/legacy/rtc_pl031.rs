...
use crate::legacy::byte_order;
use crate::legacy::time;
use crate::BusDevice;
...
impl RTC {
    ...
    pub fn new(interrupt: Arc<Box<dyn InterruptSourceGroup>>) -> RTC {
        RTC {
            ...
            tick_offset: time::get_time(time::ClockType::Real) as i64,
            match_value: 0,
            load: 0,
            imsc: 0,
            ris: 0,
    ...
    fn get_time(&self) -> u32 {
        ...
        (ts / time::NANOS_PER_SECOND as i128) as u32
    }
    ...
    fn handle_write(&mut self, offset: u64, val: u32) -> Result<()> {
        match offset {
            ...
            RTCLR => {
                ...
                self.tick_offset = time::seconds_to_nanoseconds(i64::from(val)).unwrap();
            }
            RTCIMSC => {
                self.imsc = val & 1;
...
mod tests {
    use super::*;
    use arch::layout::RTC_DEVICE_MMIO_START;
    use std::io;
    ...
    fn test_rtc_read_write_and_event() {
        ...
        rtc.write(RTC_DEVICE_MMIO_START, RTCMR, &mut data);
        rtc.read(RTC_DEVICE_MMIO_START, RTCMR, &mut data);
        let v = byte_order::read_le_u32(&data[..]);
        assert_eq!(v, 123);

        // Read and write to the LR register.
        let v = time::get_time(time::ClockType::Real);
        byte_order::write_le_u32(&mut data, (v / time::NANOS_PER_SECOND) as u32);
        let previous_now_before = rtc.previous_now;
        rtc.write(RTC_DEVICE_MMIO_START, RTCLR, &mut data);

        assert!(rtc.previous_now > previous_now_before);

        rtc.read(RTC_DEVICE_MMIO_START, RTCLR, &mut data);
        let v_read = byte_order::read_le_u32(&data[..]);
        assert_eq!((v / time::NANOS_PER_SECOND) as u32, v_read);

        // Read and write to IMSC register.
        // Test with non zero value.
        let non_zero = 1;
        byte_order::write_le_u32(&mut data, non_zero);
        rtc.write(RTC_DEVICE_MMIO_START, RTCIMSC, &mut data);
        // The interrupt line should be on.
        assert!(rtc.interrupt_evt.read().unwrap() == 1);
        rtc.read(RTC_DEVICE_MMIO_START, RTCIMSC, &mut data);
        let v = byte_order::read_le_u32(&data[..]);
        ...
        rtc.write(RTC_DEVICE_MMIO_START, RTCICR, &mut data);
        // The interrupt line should be on.
        assert!(rtc.interrupt_evt.read().unwrap() > 1);
        let v_before = byte_order::read_le_u32(&data[..]);
        let no_errors_before = METRICS.rtc.error_count.count();

        rtc.read(RTC_DEVICE_MMIO_START, RTCICR, &mut data);
        ...
        byte_order::write_le_u32(&mut data, 0);
        rtc.write(RTC_DEVICE_MMIO_START, RTCCR, &mut data);
        rtc.read(RTC_DEVICE_MMIO_START, RTCCR, &mut data);
        let v = byte_order::read_le_u32(&data[..]);
        assert_eq!(v, 1);
        ...
        byte_order::write_le_u32(&mut data, 0);
        let no_errors_before = METRICS.rtc.error_count.count();
        rtc.write(RTC_DEVICE_MMIO_START, AMBA_ID_LOW, &mut data);
        let no_errors_after = METRICS.rtc.error_count.count();
        assert_eq!(no_errors_after - no_errors_before, 1);
        // However, reading from the AMBA_ID_LOW should succeed upon read.

        let mut data = [0; 4];
        rtc.read(RTC_DEVICE_MMIO_START, AMBA_ID_LOW, &mut data);
